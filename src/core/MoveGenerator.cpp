...
namespace draughts::core {
    ...
    namespace {
        bool manReplay(const Board& b, Color side, Square cursor, Bitboard remaining,
            Square* out, int cap, int& n)
        {
            if (remaining == 0) { if (cap > 0) out[n++] = cursor; return cap > 0; }
            for (int d = 0; d < bc::kNumDirs; ++d) {
                ...
                out[n++] = landing;
                if (manReplay(b, side, landing, remaining & ~Board::bit(v), out, cap + 1, n))
                    return true;
                --n;
            }
            return false;
            ...
            Square* out, int cap, int& n)
        {
            ...
            for (int d = 0; d < bc::kNumDirs; ++d) {
                ...
                for (int k = 0; k < kBoardSize; ++k) {
                    ...
                    out[n++] = landing;
                    if (kingReplay(b, side, landing, remaining & ~Board::bit(victim), out, cap + 1, n))
                        return true;
                    --n;
                }
            }
        ...
        Square* out, int outCapacity) noexcept
    {
        ...
        const bool ok = (p.kind == PieceKind::Man)
            ? manReplay(board, side, move.from, move.captured, out, 0, n)
            : kingReplay(board, side, move.from, move.captured, out, 0, n);
        return ok;
    }
